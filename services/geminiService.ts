...
import { Transaction, ActivityGroup, TransactionType, AccountType, ReportType, TransactionStatus, FinancialRatio } from '../types';
  ...
  try {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            group: { type: Type.STRING, enum: ["OPERATING", "INVESTING", "FINANCING"] },
            type: { type: Type.STRING, enum: ["INCOME", "EXPENSE"] },
            accountType: { type: Type.STRING, enum: ["CASH", "PAYABLE", "RECEIVABLE"] }
          }
        }
      }
    });
    ...
    if (response.text) {
        ...
        return {
            ...
            status: (data.accountType === 'PAYABLE' || data.accountType === 'RECEIVABLE') ? TransactionStatus.PENDING : TransactionStatus.PAID
        };
    }
...
export const generateReportAnalysis = async (
    ...
    try {
        ...
        return {
            analysis: "Could not generate AI analysis at this time. Displaying raw calculated data.",
            ratios: calculatedRatios.map(r => ({...r, analysis: r.analysis || "Analysis unavailable."}))
        };
    }
}