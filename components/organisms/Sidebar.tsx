...
import { LayoutDashboard, Receipt, FileText, LogOut, Settings, Bell, ShieldAlert, Check, Copy } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentPage, setPage }) => {
  ...
  const handleGenerateToken = async () => {
      const code = await generateAdminToken();
      if (code) {
          setAdminCode(code);
          setIsModalOpen(true);
      }
  ...
  return (
    ...
    <div className="w-64 bg-gray-800 text-white flex flex-col h-full md:h-screen md:sticky md:top-0 no-print shadow-xl">
      ...
      <div className="p-4 border-t border-slate-700 bg-slate-900 space-y-3">
        ...
        <div className="flex items-center gap-3 px-2 pt-2">
            {currentUser?.avatar ? (
                <img src={currentUser.avatar} alt="User" className="w-8 h-8 rounded-full bg-slate-800 object-cover" />
            ) : (
    ...
    {/* Admin Code Modal */}
    <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={t('generateAdminCode')}>
        <div className="text-center">
            ...
            <div className="bg-gray-100 p-6 rounded-xl border-2 border-dashed border-gray-300 mb-4 flex items-center justify-center">
                <span className="text-4xl font-mono font-bold tracking-widest text-gray-800">{adminCode}</span>
            </div>
            <p className="text-xs text-red-500 font-semibold mb-6">{t('codeValid5Min')}</p>
            
            <div className="flex gap-2">
                <button onClick={() => setIsModalOpen(false)} className="flex-1 px-4 py-2 border rounded-lg text-gray-600 hover:bg-gray-50">
                    {t('close')}
                </button>
                <button onClick={copyToClipboard} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2">
