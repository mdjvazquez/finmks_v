...
import { Transaction, ActivityGroup, TransactionType, AccountType, TransactionStatus, UserRole } from '../../types';
...
export const TransactionForm: React.FC<TransactionFormProps> = ({ onSubmit, onCancel }) => {
  const { companySettings, currentUser, t } = useFinance();
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        try {
            ...
            setFormData(prev => ({ ...prev, ...result, receiptImage: base64String }));
        } catch (error) {
  ...
  const handleAccountTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      setFormData(prev => ({ ...prev, accountType: newType, status: newStatus }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.accountType === AccountType.PAYABLE || formData.accountType === AccountType.RECEIVABLE) {
        if (!formData.dueDate) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
            <label className="flex items-center gap-2 cursor-pointer mb-2">
                    <input type="checkbox" checked={addTax} onChange={e => setAddTax(e.target.checked)} disabled={!canEdit} className="rounded text-blue-600 focus:ring-blue-500 bg-white"/>
        ...
        <div className="grid grid-cols-2 gap-4">
            <Select 
                ...
                options={Object.values(AccountType).map(tVal => ({value: tVal, label: t(`val_${tVal}`)}))}
            />
            <Select 
                label={t('flowType')} 
                value={formData.type} 
                ...
                options={Object.values(TransactionType).map(tVal => ({value: tVal, label: t(`val_${tVal}`)}))}
            />
        </div>
        ...
        {(formData.accountType === AccountType.PAYABLE || formData.accountType === AccountType.RECEIVABLE) && (
            <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-100 grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <Select 
                        ...
                        disabled={true} // Forced to Pending initially by requirement logic, but usually editable later in list
                        options={Object.values(TransactionStatus).map(s => ({value: s, label: t(`val_${s}`)}))} 
                    />
            </div>
        )}

        <Select label={t('activityGroup')} value={formData.group} onChange={e => setFormData({...formData, group: e.target.value as ActivityGroup})} disabled={!canEdit} options={Object.values(ActivityGroup).map(g => ({value: g, label: t(`val_${g}`)}))} />

        <div className="flex gap-3 pt-4">
            <Button type="button" variant="secondary" onClick={onCancel} className="flex-1">{t('cancel')}</Button>
            <Button type="submit" disabled={!canEdit} className="flex-1">{t('saveMovement')}</Button>
        </div>
    </form>
  );
};
