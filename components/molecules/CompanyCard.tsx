...
import { Save, Edit2, Upload, Building2, CreditCard, MapPin, Percent } from 'lucide-react';
...
export const CompanyCard: React.FC<CompanyCardProps> = ({ settings, onUpdate, t }) => {
            ...
            {!isEditing ? (
                 ...
                 <div className="flex gap-2">
                     <Button 
                        ...
                        onClick={() => { setIsEditing(false); setFormData(settings); }}
                     >
                        {t('cancel')}
                     </Button>
                     <Button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
            ...
            <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex-1">
                    <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">{t('taxId')}</p>
                    {isEditing ? (
                        <input 
                            type="text" 
                        ...
                        <p className="text-sm font-medium text-gray-900">{settings.taxId}</p>
                    )}
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex-1">
                    <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">{t('taxRate')}</p>
                    {isEditing ? (
                        <input 
                            type="number" 
                        ...
                        <p className="text-sm font-medium text-gray-900">{settings.taxRate}%</p>
                    )}
                    <p className="text-[10px] text-gray-400 mt-1">{t('taxRateDesc')}</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg md:col-span-2">
                ...
                <div className="flex-1">
                    <p className="text-xs text-gray-500 uppercase font-bold tracking-wider">{t('businessAddress')}</p>
                    {isEditing ? (
                        <input 
                            type="text" 
