...
interface FinancialContextType {
  ...
  addNewUser: (user: Partial<User>) => Promise<{success: boolean, code?: string}>;
...
export const FinancialProvider = ({ children }: { children: ReactNode }) => {
  ...
  const createInvitation = async (email: string, name: string, role: string) => {
      ...
      if (error) {
          ...
          if (error.code === '42501') {
              alert("Permission Denied: Admins need 'insert' access to invitations table.");
          }
          return null;
      }
      return code;
  ...
  const verifyInvitation = async (code: string) => {
      const { data, error } = await supabase.from('invitations').select('*').eq('code', code).single();
  ...
  const seedInitialData = async (userId: string) => {
    ...
    const descriptions = [
        { d: "Consulting Services", t: TransactionType.INCOME, g: ActivityGroup.OPERATING, a: AccountType.RECEIVABLE },
        { d: "Office Rent", t: TransactionType.EXPENSE, g: ActivityGroup.OPERATING, a: AccountType.PAYABLE },
        { d: "Server Hosting", t: TransactionType.EXPENSE, g: ActivityGroup.OPERATING, a: AccountType.PAYABLE },
        { d: "New Laptop", t: TransactionType.EXPENSE, g: ActivityGroup.INVESTING, a: AccountType.CASH },
        { d: "Client Retainer", t: TransactionType.INCOME, g: ActivityGroup.OPERATING, a: AccountType.CASH },
    ];

    for (let i = 0; i < 20; i++) { 
        const tmpl = descriptions[Math.floor(Math.random() * descriptions.length)];
        const isPast = Math.random() > 0.2; 
        const date = new Date(today);
        date.setDate(today.getDate() + (isPast ? -Math.floor(Math.random() * 90) : Math.floor(Math.random() * 30)));
        
        let status = TransactionStatus.PAID;
        let dueDate = null;

        if (tmpl.a === AccountType.PAYABLE || tmpl.a === AccountType.RECEIVABLE) {
            status = Math.random() > 0.5 ? TransactionStatus.PAID : TransactionStatus.PENDING;
  ...
  return (
    <FinancialContext.Provider value={{
      currentUser, allUsers, login, signUp, logout, updateUserProfile, requestPasswordReset, confirmPasswordChange,
      ...
      fetchAllUsers, addNewUser, toggleUserStatus, createInvitation, verifyInvitation
    }}>
      {children}
