...
export const Dashboard: React.FC = () => {
  ...
  const filteredTransactions = useMemo(() => {
    // Exclude Internal Transfers from Dashboard Metrics
    const financialTx = transactions.filter(
      (t) => t.type !== TransactionType.TRANSFER
    );

    if (!dateRange.start || !dateRange.end) return financialTx;

    const start = new Date(dateRange.start);
    const end = new Date(dateRange.end);
    end.setHours(23, 59, 59, 999);

    return financialTx.filter((t) => {
  ...
  const metrics = useMemo(() => {
    ...
    const receivables = filteredTransactions
      .filter((t) => t.accountType === AccountType.RECEIVABLE)
      .reduce((acc, t) => acc + t.amount, 0);
    const payables = filteredTransactions
      .filter((t) => t.accountType === AccountType.PAYABLE)
      .reduce((acc, t) => acc + t.amount, 0);
