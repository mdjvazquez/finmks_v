...
import { Save, Building2, UserCircle, Globe, Shield, KeyRound, Loader2, Settings2, Users, UserPlus, Power, Mail, Copy, Check } from 'lucide-react';

export const Settings: React.FC = () => {
  const { companySettings, updateCompanySettings, currentUser, updateUserProfile, requestPasswordReset, confirmPasswordChange, t, allUsers, addNewUser, toggleUserStatus } = useFinance();
      ...
      <div className="space-y-8">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-8">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <h3 className="text-lg font-semibold mb-6 flex items-center gap-2 text-gray-800 border-b pb-2">
                        <UserCircle className="text-purple-500" size={20}/> {t('myProfile')}
                    </h3>
        ...
        {activeTab === 'COMPANY' && isAdmin && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <form onSubmit={handleSaveCompany} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <h3 className="text-lg font-semibold mb-6 flex items-center gap-2 text-gray-800 border-b pb-2">
                        <Building2 className="text-blue-500" size={20}/> {t('companyInfo')}
                    </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="flex flex-col items-center justify-center space-y-3 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                            {settingsForm.logoUrl ? (
                                <img src={settingsForm.logoUrl} alt="Logo" className="h-24 object-contain" />
                            ) : (
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">{t('taxRateDesc')}</p>
                        </div>
                    </div>
                                    ...
                                    return (
                                        <tr key={user.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 font-medium text-gray-900 flex items-center gap-3">
                                                ...
                                                {user.name} {isCurrentUser && <span className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded ml-2">(You)</span>}
                                            </td>
                                            <td className="px-6 py-4 text-gray-600">{user.email}</td>
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                    user.role === UserRole.ADMIN ? 'bg-purple-100 text-purple-800' :
                                                    user.role === UserRole.ACCOUNTANT ? 'bg-blue-100 text-blue-800' :
                                            ...
                                            <td className="px-6 py-4">
                                                <button
                                                    onClick={() => !isCurrentUser && toggleUserStatus(user.id, user.status || 'ACTIVE')}
                        ...
                        {resetStep === 1 ? (
                            ...
                            <div className="w-full space-y-4">
                                ...
                                <button 
                                    ...
                                    disabled={isLoading || verificationCode.length !== 6 || newPassword.length < 3}
