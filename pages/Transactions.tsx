...
import { Transaction, TransactionType, UserRole, TransactionStatus } from '../types';
import { Trash2, CheckCircle, Clock, AlertTriangle, Plus, ShieldAlert } from 'lucide-react';
...
export const Transactions: React.FC = () => {
  const { transactions, addTransaction, deleteTransaction, updateTransactionStatus, currentUser, t, isLoading } = useFinance();
  ...
  const executeAction = async () => {
      ...
      if (confirmConfig.actionType === 'SAVE') {
          const result = await addTransaction(confirmConfig.payload);
          success = result.success;
          message = t('transactionSaved');
          errorMsg = result.error || t('transactionError');
      
      } else if (confirmConfig.actionType === 'STATUS') {
          // updateTransactionStatus is currently void in context but async. Assuming success for UI.
          // Ideally context should return success status. We'll wait for it.
          await updateTransactionStatus(confirmConfig.payload.id, confirmConfig.payload.status);
          success = true;
          message = "Status updated successfully"; // Need translation?
  ...
  const confirmRestrictedDelete = async () => {
      ...
      if (result.success) {
          setStatusModal({ isOpen: true, type: 'success', message: t('transactionDeleted') });
          setTimeout(() => setStatusModal(prev => ({...prev, isOpen: false})), 2000);
      } else {
          setStatusModal({ isOpen: true, type: 'error', message: result.error || t('invalidAdminCode') });
      }
  };
          ...
          return (
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 text-sm space-y-2 text-left">
                ...
                <div className="flex justify-between">
                    ...
                    <span className="font-bold text-blue-600 text-lg">${data.amount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                    ...
                    <span className="text-gray-900">{t(`val_${data.accountType}`)}</span>
                </div>
                 <div className="flex justify-between">
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col justify-between items-start mb-8 gap-4 md:flex-row md:items-center">
        ...
        <div className="flex flex-col gap-4 w-full md:w-auto items-end">
             <Button onClick={() => setIsFormOpen(true)} icon={<Plus size={18}/>} className="w-full md:w-auto">
                {t('newTransaction')}
             </Button>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                        ...
                        <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4">
                                <div className="flex flex-col gap-1 items-start">
                                    <Badge variant="neutral">{t(`val_${tx.accountType}`)}</Badge>
                                    <Badge variant="info">{t(`val_${tx.group}`)}</Badge>
                                </div>
                            </td>
                            <td className="px-6 py-4">
                                {tx.status === TransactionStatus.PAID ? (
                                    <Badge variant="success"><CheckCircle size={10} /> {t('paid')}</Badge>
                                ) : (
                                    <div className="flex flex-col gap-1 items-start">
                                        <Badge variant="warning"><Clock size={10} /> {t('pending')}</Badge>
                                        {tx.dueDate && <span className="text-xs text-red-500 flex items-center gap-1"><AlertTriangle size={10} /> {tx.dueDate}</span>}
                                    </div>
                                )}
                            </td>
                            ...
                            <td className="px-6 py-4 text-center">
                                <div className="flex items-center justify-center gap-2">
                                    {canEdit && tx.status === TransactionStatus.PENDING && (
                                        <button 
                                            onClick={() => initiateStatusChange(tx.id, TransactionStatus.PAID)} 
                    ...
                    {filteredTransactions.length === 0 && (
                        <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-400">{t('noTransactions')}</td></tr>
                    )}
                </tbody>
            </table>
        </div>
      ...
      <Modal
        ...
        onClose={() => { setIsAdminAuthOpen(false); setAdminAuthCode(''); }}
      ...
      >
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-gray-600 mb-4 text-sm">{t('deleteRestrictedMsg')}</p>
            
            <div className="w-full mb-6">
                <Input 
                    placeholder="XXXX" 
                    value={adminAuthCode}
                    onChange={e => setAdminAuthCode(e.target.value.toUpperCase().trim())}
                    maxLength={4}
            ...
            <div className="flex w-full gap-3">
                <Button variant="ghost" onClick={() => setIsAdminAuthOpen(false)} className="flex-1">
                    {t('cancel')}
                </Button>
                <Button variant="danger" onClick={confirmRestrictedDelete} disabled={adminAuthCode.length !== 4} className="flex-1">
                    {t('confirmDelete')}
                </Button>
            </div>
          </div>
